...
async function main() {
    ...
    try {
        const migrationPath = path.join(__dirname, '../database/community_upgrade.sql');
        const statements = parseStatements(fs.readFileSync(migrationPath, 'utf8'));
        for (const statement of statements) await connection.query(statement);
        console.log(`Community migration applied (${statements.length} statements).`);
    } finally {
...
main().catch((error) => {
    console.error('Community migration failed:', error.sqlMessage || error.message);
    process.exitCode = 1;
});
